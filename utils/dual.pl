#!/usr/bin/perl
# generating the vlp file for the dual approximation
# of the 3-dimensional visualization
#   dual.pl <output> <ineq1> <ineq2> ...
#
############################################################
use strict;

sub print_usage {
    print "usage: dual.pl <output> <ineq1> <ineq2> ...\n",
...
# a[0]*h1 + ... + a[10]*h11 >=0
#  the coeff of h10 and h11 is zero (to be checked)
#  A*(-h1) <= B*[(h2+h5)/2+h10] + C*[(h8+h9)/2+h11] + 
#           D*[(h3+h4+h6+h7)/4]
#  a1*h1+(a2+a5)*(h2+h5)/2 + (a8+a9)*(h8+h9)/2 + (a3+a4+a6+a7)*()/4
#  A = a[0]
#  B = a[1]+a[4]
#  C = a[7]+a[8]
#  D = a[2]+a[3]+a[5]+a[6]
...
sub create_vlp {
    ...
    print VLP "c outer bound for the dual region from inequalities in\n";
